...
- competition_points:
    * Base: 4 for a win, 2 for a draw, 0 for a loss
    * Losing bonus: +1 for a loss by 7 points or fewer (all comps for now)
    * Try bonus: placeholder (0 for now, TSDB doesn't expose tries)

We also track (if columns exist in the DB):
...
- If it DOES exist, we introspect its columns and:
    * Add losing_bonus_points and try_bonus_points if missing.
    * Only read/write columns that actually exist.
...
def _ensure_team_season_stats_table(cur, verbose: bool = False) -> None:
    ...
    If it exists, ensure losing_bonus_points and try_bonus_points columns exist.
    """
    if not _table_exists(cur, "team_season_stats"):
        if verbose:
    ...
    if "losing_bonus_points" not in cols:
        if verbose:
            print("[INFO] Adding losing_bonus_points column to team_season_stats")
        cur.execute(
            """
            ALTER TABLE team_season_stats
            ADD COLUMN IF NOT EXISTS losing_bonus_points INTEGER NOT NULL DEFAULT 0;
            """
        )

    if "try_bonus_points" not in cols:
        if verbose:
            print("[INFO] Adding try_bonus_points column to team_season_stats")
        cur.execute(
            """
            ALTER TABLE team_season_stats
            ADD COLUMN IF NOT EXISTS try_bonus_points INTEGER NOT NULL DEFAULT 0;
            """
        )
...
# For now, use a single global losing bonus margin: 7 points
...
class TeamSeasonAgg:
    ...
    losing_bonus_points: int = 0
    try_bonus_points: int = 0  # placeholder, currently always 0
...
def _aggregate_team_season_stats(
    ...
    """
    Aggregate per (league_id, season_id, team_id), including losing bonus points.
    """
    ...
    for m in matches:
        ...
        if hs > as_:
            ...
            # Base points
            home.competition_points += 4  # win
            # Losing bonus for away if within margin
            if margin <= LOSING_BONUS_MARGIN:
            ...
            # Base points
            away.competition_points += 4  # win
            # Losing bonus for home if within margin
            if margin <= LOSING_BONUS_MARGIN:
            ...
            # No losing bonus in a draw

        # try_bonus_points: placeholder, could be filled later if we know tries
    ...
    for key, stat in agg.items():
        if verbose:
            ...
            print(
                ...
                f"(LB={stat.losing_bonus_points}, TB={stat.try_bonus_points})"
            )
        ...
        add("losing_bonus_points", stat.losing_bonus_points)
        add("try_bonus_points", stat.try_bonus_points)
        ...
        def add_update(name: str):
            if name in cols:
                set_fragments.append(f"{name} = EXCLUDED.{name}")

        add_update("games_played")
        add_update("wins")
        add_update("draws")
        add_update("losses")
        add_update("points_for")
        add_update("points_against")
        add_update("points_diff")
        add_update("competition_points")
        add_update("losing_bonus_points")
        add_update("try_bonus_points")
        if "updated_at" in cols:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Compute season-level stats for each team from matches (with losing bonus points)."
    )
    parser.add_argument(
        "--only-tsdb-league",
    ...
    try:
        ...
        if verbose:
            print("[DONE] team season stats computed and upserted (with losing bonus points).")
