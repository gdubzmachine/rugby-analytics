...
- For each event (match) it:
    ...
    * Maps TSDB venue ID to DB venues via:
        venues.tsdb_venue_id  (OPTIONAL – if your schema doesn't have it, we skip gracefully)
    * Inserts/updates `matches` using a natural key:
        (league_id, season_id, home_team_id, away_team_id, kickoff_utc)

If the league row for this TSDB league_id does *not* exist yet in `leagues`,
the script will:
- Try to find a row by name/short_name (case-insensitive)
- If found: update that row to set tsdb_league_id
- If not found: insert a new league row.

If a season row for (league_id, tsdb_season_key) does *not* exist yet in `seasons`,
the script will:
- INSERT ... ON CONFLICT (league_id, year) DO UPDATE ...
  and return season_id (robust against re-runs).

If your `venues` table does *not* have a `tsdb_venue_id` column,
we **do not** raise – we detect this once and set venue_id = NULL for all rows.
...
def _lookup_league_id(cur, tsdb_league_id: str, cache: Dict[str, int]) -> int:
    """
    Find league_id by tsdb_league_id. Raises if not found.
    """
...
def _ensure_league_in_db(
    ...
    """
    If no league row exists with this tsdb_league_id, try to:
      - Find an existing row by name/short_name and update tsdb_league_id
      - Otherwise insert a new league row.
    """
    ...
    if not name:
        raise RuntimeError(
            f"Cannot ensure league tsdb_league_id={tsdb_league_id}: league_meta missing strLeague"
        )
    ...
    # 1) Try to find existing league by name/short_name (case-insensitive)
    cur.execute(
        """
        SELECT league_id
        FROM leagues
    ...
    if row:
        league_id = row[0]
        if verbose:
            print(f"[INFO] Found existing league_id={league_id} by name/short_name; updating tsdb_league_id")
        cur.execute(
            """
            UPDATE leagues
    ...
    # 2) Need to insert a new league row.
    #    We need sport_id; best effort: pick rugby_union / rugby in sports.
    cur.execute(
        """
        SELECT sport_id
        FROM sports
    ...
    if not sport_row:
        raise RuntimeError(
            "Cannot insert new league row: no rugby sport found in sports table "
            "(expected code='rugby_union' or name ILIKE 'rugby%')."
        )
    sport_id = sport_row[0]

    if verbose:
        print(f"[INFO] Inserting new league row for tsdb_league_id={tsdb_league_id}, sport_id={sport_id}")
...
def _ensure_or_get_season_id(
    ...
    """
    Find or create a season row for (league_id, tsdb_season_key).
    Uses INSERT ... ON CONFLICT (league_id, year) DO UPDATE ... RETURNING season_id.
    """
    key = (league_id, tsdb_season_key)
    if key in cache:
        return cache[key]

    # Try existing row by tsdb_season_key first
    cur.execute(
        """
        SELECT season_id
        FROM seasons
        WHERE league_id = %s AND tsdb_season_key = %s
        """,
    ...
    # Insert (or upsert) season row
    def _parse_year(lbl: str) -> Optional[int]:
        if not lbl:
            return None
        try:
            return int(lbl[:4])
    ...
    base_year = _parse_year(tsdb_season_key)

    if verbose:
        print(
            f"[INFO] Ensuring season row for league_id={league_id}, "
            f"tsdb_season_key='{tsdb_season_key}', year={base_year}"
        )

    # ON CONFLICT requires that (league_id, year) is a unique index as per your schema.
    cur.execute(
        """
        INSERT INTO seasons (
            league_id,
...
# Cache whether venues.tsdb_venue_id exists (None = unknown, True/False = probed)
_VENUES_HAS_TSDB_COLUMN: Optional[bool] = None

def _venues_has_tsdb_column(cur) -> bool:
    """
    Check once whether the venues table has a tsdb_venue_id column, and cache the result.
    """
...
def _lookup_venue_id(cur, tsdb_venue_id: Optional[str],
                     cache: Dict[str, int]) -> Optional[int]:
    """
    Map TSDB venue ID → venues. If the tsdb_venue_id column doesn't exist in your schema,
    skip venue mapping without error (return None).
    """
    ...
    if not _venues_has_tsdb_column(cur):
        # Schema doesn't support TSDB mapping yet; leave NULL
        return None
    ...
    if row:
        ...
        cur.execute(
            ...
            (
                ...
                attendance,
                venue_id,
                "other",  # data_source enum; using 'other' for TheSportsDB
                match_id,
            ),
        )
    else:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="How many seasons backwards from league.strCurrentSeason "
             "(default: 10; e.g. 10 → current + previous 9 seasons).",
    )
    parser.add_argument(
        "--api-key",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Sleep between seasons to be nice to the API (default: 0.5).",
    )
    parser.add_argument(
        "--write-csv",
        action="store_true",
    ...
    # 1) Lookup league & current season label
    ...
    if not current_season_label:
        raise SystemExit(
            "League metadata does not include strCurrentSeason; "
            "you may need to specify seasons manually in a custom script."
        )

    # 2) Build synthetic seasons list: [current, prev, prev, ...]
    ...
    # Resolve / ensure DB league_id from tsdb_league_id
    try:
        league_id_db = _lookup_league_id(cur, league_id_tsdb, league_id_cache)
    except RuntimeError:
        # Try to attach or insert league row using TSDB metadata
        if verbose:
            print(
                f"[INFO] No league row with tsdb_league_id={league_id_tsdb} found. "
                f"Attempting to attach/insert using TSDB metadata..."
            )
    ...
    try:
        for idx, season in enumerate(seasons_to_fetch, start=1):
            if verbose:
                print(f"[INFO] ({idx}/{len(seasons_to_fetch)}) Fetching events for TSDB season={season!r}")
            ...
            if not events:
                if verbose:
                    print(f"[WARN]  -> no events for season={season}, skipping DB ingest")
                continue
            ...
            # Resolve or create DB season_id (robust, upsert-style)
            ...
            for e in events:
                ...
                if home_team_id is None or away_team_id is None:
                    if verbose:
                        print(
                            f"  [SKIP] Missing team(s) for event idEvent={e.get('idEvent')}: "
                            f"home_tsdb={id_home_tsdb}, away_tsdb={id_away_tsdb}"
                        )
                    total_skipped += 1
                    continue

                # Resolve venue (may be None if tsdb_venue_id column doesn't exist)
                ...
                # Check if match exists to count insert/update
                cur.execute(
                    """
                    SELECT match_id
                    FROM matches
