...
CSV_PATH = ROOT_DIR / "data" / "six_nations_2023_matches.csv"
COMPETITION_NAME = "Six Nations 2023"


def load_matches_from_csv() -> pd.DataFrame:
    print(f"Loading matches from: {CSV_PATH}")
    # Using latin1 to be safe on Windows; can switch to utf-8 if the file is saved as UTF-8
...
def ensure_competition(conn) -> int:
    """
    Ensure a row exists in competitions for 'Six Nations 2023'
    ...
    conn.execute(
        text("""
            ...
            VALUES (:name, '6N 2023', 'international', 'Europe', 'csv_manual')
...
def upsert_matches(engine, df: pd.DataFrame):
    with engine.begin() as conn:
        ...
        print(f"\nUpserting {len(df)} Six Nations matches...")
    ...
    print("\n Six Nations 2023 match upsert complete.")
