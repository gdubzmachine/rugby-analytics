...
CSV_PATH = ROOT_DIR / "data" / "rwc_warmups_2023_matches.csv"
COMPETITION_NAME = "RWC Warm-ups 2023"
...
def ensure_competition(conn) -> int:
    """
    Ensure a row exists in competitions for 'RWC Warm-ups 2023'
    ...
    conn.execute(
        text("""
            ...
            VALUES (:name, 'RWC WU 2023', 'international', 'global', 'csv_manual')
...
def upsert_matches(engine, df: pd.DataFrame):
    with engine.begin() as conn:
        ...
        print(f"\nUpserting {len(df)} RWC warm-up matches...")
    ...
    print("\n RWC Warm-ups 2023 match upsert complete.")
