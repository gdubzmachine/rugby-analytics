...
CSV_PATH = ROOT_DIR / "data" / "rugby_championship_2023_matches.csv"
COMPETITION_NAME = "Rugby Championship 2023"
...
def ensure_competition(conn) -> int:
    """
    Ensure a row exists in competitions for 'Rugby Championship 2023'
    ...
    conn.execute(
        text("""
            ...
            VALUES (:name, 'RC 2023', 'international', 'Southern Hemisphere', 'csv_manual')
...
def upsert_matches(engine, df: pd.DataFrame):
    with engine.begin() as conn:
        ...
        print(f"\nUpserting {len(df)} Rugby Championship matches...")
    ...
    print("\n Rugby Championship 2023 match upsert complete.")
